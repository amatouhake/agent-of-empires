...
//   - Confirm button fires DELETE /api/sessions/{id} with the exact-session
//     cleanup body and removes the row.
  ...
  base("Delete button fires DELETE /api/sessions/{id} and removes the row", async ({ page }, testInfo) => {
    ...
    try {
      ...
      const deletePromise = page.waitForResponse(
        (res) =>
          res.url().endsWith(`/api/sessions/${encodeURIComponent(sessionId)}`) && res.request().method() === "DELETE",
      );

      // `aoe add` does not produce a managed worktree, so the dialog
      // skips the checkbox section and the exact-session request body is
      // all-false cleanup options.
      ...
      await expect
        .poll(async () => (await listSessions(serve.baseUrl)).some((session) => session.id === sessionId), {
          timeout: 10_000,
        })
        .toBe(false);
